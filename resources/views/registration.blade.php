  ...
  <body>
    <div class="container">
        <form action="{{$url}}" method="post">
          @csrf
        <div class="col-auto">
            <div class="align-middle"><h2>{{$title}}<h2></div>
            <label for="firstname" class="form-label">First Name</label>
            <input type="name" name="firstname" class="form-control" id="firstname" value="{{$customer->first_name}}" >  
            @error('firstname')
            <span class="text-warning" role="alert">{{ $message }}</span>
            @enderror
            <br>
            <label for="lastname" class="form-label">Last Name</label>
            <input type="name" name="lastname" class="form-control" id="lastname" aria-describedby="emailHelp" value="{{$customer->last_name}}">
            @error('lastname')
            <span class="text-warning" role="alert">{{ $message }}</span>
            @enderror
        </div>
        ...
        <button class="btn btn-primary me-md-2"> view</button>
        </a>
        </div>
